  ...
  return (
    <Fragment>
      {images && images.length > 0 && (
        <div
          className={`Gallery ${isStacked ? 'Gallery-column' : 'Gallery-row'}`}
        >
          {images.map(
            (image, index) =>
              index < 3 && (
      ...
      {isOpen && sliderImages.length > 0 && (
        <Lightbox
          ...
          prevSrc={
            sliderImages[
              (activeImageIndex + sliderImages.length - 1) % sliderImages.length
            ]
          }
          ...
          onMoveNextRequest={() =>
            setActiveImageIndex((activeImageIndex + 1) % sliderImages.length)
          }
